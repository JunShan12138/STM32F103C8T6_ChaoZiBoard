...
/* 输入寄存器 */
uint16_t g_mb_input_reg[MB_INPUT_REG_SUM];

/***************************************************************************************************
 * @fn      mb_read_input_reg
 *
 * @brief   功能码04H 读输入寄存器
 *
 * @param   recv 接收到的数据
 ...
 * @return  无
 */
mb_resp_e mb_read_input_reg(uint8_t *recv, uint8_t buf_len)
{
...
#if MB_INPUT_REG_START == 0
    if ((reg_start + reg_sum) > (MB_INPUT_REG_START + MB_INPUT_REG_SUM))
#else
    if ((reg_start < MB_INPUT_REG_START) || 
        ((reg_start + reg_sum) > (MB_INPUT_REG_START + MB_INPUT_REG_SUM)))
#endif
    {
    ...
    for (i = 0; i < reg_sum; i++)
    {
        tx_buf[tx_cnt++] = (uint8_t)(g_mb_input_reg[reg_index] >> 8);
        tx_buf[tx_cnt++] = (uint8_t)(g_mb_input_reg[reg_index] & 0xFF);
        reg_index++;
    }
